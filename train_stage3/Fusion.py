...
import torch
from utils import CBR, DWConv, MLP, MLP2
...
class SemLA_Fusion(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_fuse = torch.cat((feat1, feat2, feat3, feat4), dim=1)

        feat_fuse = self.fuse5(feat_fuse)
        feat_fuse = self.acitve(feat_fuse)
        result = (feat_fuse + 1) / 2
